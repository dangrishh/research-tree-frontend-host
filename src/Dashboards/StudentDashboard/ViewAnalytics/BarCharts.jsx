  ...
  useEffect(() => {
    if (data.length === 0) {  // Only fetch if the data is empty
      const fetchPdfDetailsCount = async () => {
        try {
          const response = await axios.get('https://researchtree-backend-heroku-1f677bc802ae.herokuapp.com/api/student/PdfKeywordsCount'); // Adjust the endpoint as needed
