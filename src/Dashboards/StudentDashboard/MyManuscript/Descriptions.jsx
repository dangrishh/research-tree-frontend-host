...
const ResearchCard = () => {
  ...
  const markTaskAsCompleted = async (taskId) => {
    try {
      const response = await fetch(`https://researchtree-backend-heroku-1f677bc802ae.herokuapp.com/api/student/mark-task/${taskId}`, {
        method: 'PATCH',
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`,
        },
  ...
  const handleSaveProposalTitle = async () => {
    try {
      const response = await fetch(`https://researchtree-backend-heroku-1f677bc802ae.herokuapp.com/api/student/update-proposal-title/${user._id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
