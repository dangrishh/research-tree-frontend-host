...
function App() {
  ...
  const getPdf = async () => {
    ...
    setError("");
    try {
      const result = await axios.get("https://researchtree-backend-heroku-1f677bc802ae.herokuapp.com/get-files");
      if (result?.data?.data) {
  ...
  const submitImage = async (e) => {
    ...
    try {
      const result = await axios.post(
        "https://researchtree-backend-heroku-1f677bc802ae.herokuapp.com/api/advicer/upload-files", 
        formData,
  ...
  const showPdf = (pdf) => {
    setPdfFile(`https://researchtree-backend-heroku-1f677bc802ae.herokuapp.com/files/${pdf}`);
  };

  const handleSearch = async () => {
    try {
      const result = await axios.post("https://researchtree-backend-heroku-1f677bc802ae.herokuapp.com/search", { searchQuery });
