...
export default function GradingTable({ studentId, panelistId }) {
  ...
  useEffect(() => {
    const fetchRubrics = async () => {
      try {
        const response = await axios.get('https://researchtree-backend-heroku-1f677bc802ae.herokuapp.com/api/advicer/fetch-rubrics');
  ...
  const handleSubmitGrades = async () => {
    ...
    try {
      const response = await axios.post(
        'https://researchtree-backend-heroku-1f677bc802ae.herokuapp.com/api/advicer/submit-student/grade',
        payload,
