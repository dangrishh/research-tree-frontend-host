...
export default function GradingTable({ studentId }) {
    ...
    const fetchGrades = async () => {
      try {
        const response = await axios.get(
          `https://researchtree-backend-heroku-1f677bc802ae.herokuapp.com/api/advicer/fetch/adviser-student/grades/${user._id}`
        );
  ...
  const fetchFinalGrade = async () => {
    try {
      const response = await axios.get(
        `https://researchtree-backend-heroku-1f677bc802ae.herokuapp.com/api/advicer/fetch/adviser-FinalGrades/grades/${user._id}/${selectedRubricId}` // Include selectedRubricId in the endpoint
      );
